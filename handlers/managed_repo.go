...
func CreateManagedGroup(c *gin.Context) {
	...
	if req.ParentID != nil && *req.ParentID > 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "仅允许在系统内引入顶层根组。子组必须先在托管平台上创建，再通过父组同步按钮递归拉取同步。"})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("该组路径在托管平台上不存在，无法引入: %v", err)})
		return
	}
...
func CreateManagedRepo(c *gin.Context) {
	...
	// 1. 调用远程服务物理创建仓库
	remoteID, sshURL, httpURL, err := services.CreateRemoteRepo(c.Request.Context(), req.Name, group.FullPath, group.ID, req.Tags, req.Description)
	if err != nil {
